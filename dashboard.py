...
st.set_page_config(page_title= "My Streamlit Web App", page_icon= "📊", layout= "wide")
...
if data is not None:
    ...
    os.chdir(r"C:\Users\asus\Streamlit")  
    df = pd.read_csv("Nassau Candy Distributor.csv", encoding= "ISO-8859-1") 
...
# Create for State
state = st.sidebar.multiselect("Pick the State", df["State/Province"].unique())
# if not state:
#     df3 = df2.copy()
# else:
...
ship_mode = st.sidebar.multiselect("Pick Ship Mode",df["Ship Mode"].unique())
...
def ship_mode(filtered_df):
    ...
    fig.update_layout(
    title={
        'text': "<b>Lead Time vs Ship Mode</b>",
        'x': 0.5,
        'xanchor': 'center',
        'font': dict(size=24)})
...
def drilldown(filtered_df):
    ...
    'Order ID':'count',
    'Sales':'sum',
    'Gross Profit':'sum'}).sort_values('Shipping Lead Time', ascending=True).round(2).head(15).reset_index()
...
#Fig1
    ...
    fig1.update_layout(
    title={
        'text': "<b>📍 State-Level Performance Insights</b>",
        'x': 0.5,
        'xanchor': 'center',
        'font': dict(size=24)})
